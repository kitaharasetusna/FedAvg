...
import torch
from my_utils.language_utils import word_to_indices, letter_to_vec
...
class ShakeSpeare(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # indices: 当前句子的int的list, target 下一个字母的数字
...
def read_dir(data_dir):
    clients = []
    groups = []
    data = defaultdict(lambda: None) # set default values for all keys

    files = os.listdir(data_dir)
    files = [f for f in files if f.endswith('.json')] # ['all_data_niid_2_keep_0_train_9.json']
    for f in files:
        file_path = os.path.join(data_dir, f) # './shakespeare/train/all_data_niid_2_keep_0_train_9.json'
    ...
    # clients: 人物; gropus: 作品; data: 对话(人物： 对话)
