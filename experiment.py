...
def fedavg_MNIST_TwoLayerNet_acc_paint_plot():
    my_path = 'data\\test_acc\MNIST\MNIST_TwoLayerNet_fedavg_num_Client_100_lr_0.001\\'
    E = [1, 5, 20]
    B = [600, 10, 50]
    for e in E:
        for b in B:
...
def grid_searching_MNIST_epoch_100(E, B, train=True):
    ...
    grid_arr = np.round(grid_arr, decimals=3)
    ...
    ax.set_xticklabels([0.5*i for i in range(-2, 2, 1)])
    ax.set_yticklabels([0.5*i for i in range(-2, 3, 1)])
